...
class mainCog(commands.Cog): 
	...
	async def join_(self, ctx):
		...
		if basicSetting[7] == "":
			...
			if basicSetting[6] != "":
				voice_client1 = await ctx.guild.get_channel(basicSetting[6]).connect(reconnect=True)
	...
	async def itemList_(self, ctx, *, args : str = None):
		if ctx.message.channel.id == basicSetting[7] or ctx.message.channel.id == basicSetting[22]:
			...
			if not args:
				...
				embed_index : int = 0
				embed_cnt : int = 0
				embed = discord.Embed(title = '', description = f'`{ctx.user.name}\'s 창고`', color = 0x00ff00)
